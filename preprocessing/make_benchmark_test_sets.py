    ...
    for featurization in FEATURIZATIONS:
        df = pd.concat(
            [
                ...
                .sample(600, replace=True, random_state=SEED)
        ...
        smallest_5_percent = pd.concat([smallest_5_percent for _ in range(3)])
        largest_5_percent = df[math.ceil(0.95 * df.shape[0]) :]
        largest_5_percent = pd.concat([largest_5_percent for _ in range(3)])

        Path("data/preprocessed/time_benchmark").mkdir(parents=True, exist_ok=True)
        df.to_csv(
            f"data/preprocessed/time_benchmark/representative.{featurization}.csv"
        )
        pd.concat([df, df]).to_csv(
            f"data/preprocessed/time_benchmark/representative_doubled.{featurization}.csv"
        )
