...
def main(use_prefit: bool):
    ...
    for dataset, featurization in it.product(DATASETS, FEATURIZATIONS):
        ...
        leaderboard_file = f"output/autogluon_extreme/leaderboards/leaderboard.{config_id_str}.csv"
        train_time_file = f"output/autogluon_extreme/leaderboards/train_time.{config_id_str}.txt"
        top_ensemble_leaderboard_file = (
            f"output/autogluon_extreme/leaderboards/leaderboard.{config_id_str}.top_ensemble.csv"
        )
        predictor_file = f"output/autogluon_extreme/models/model.{config_id_str}"
        top_performing_metrics_file = (
            f"top_models/autogluon_extreme/top_model.{config_id_str}.csv"
        )
        for directory in ["leaderboards", "models"]:
            Path(f"output/autogluon_extreme/{directory}").mkdir(exist_ok=True, parents=True)
        Path(f"top_models/autogluon_extreme").mkdir(exist_ok=True, parents=True)
